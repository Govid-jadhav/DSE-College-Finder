...
def parse_full_pdf():
    ...
    # Regex for percentage: (XX.XX%)
    pct_pattern = re.compile(r"^\(\d+\.\d+%\)$")
    # Regex for rank: positive integer
    rank_pattern = re.compile(r"^\d+$")
    
    for page_idx in range(total_pages):
        if (page_idx + 1) % 50 == 0 or page_idx == 0 or page_idx == total_pages - 1:
    ...
    print(f"Finished parsing. Total choice codes parsed: {len(colleges)}")
    
    # Save output to JSON
    print(f"Saving data to {output_path}...")
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(colleges, f, separators=(',', ':'))
